...
struct Stats {
    ...
    total_model_diffs: u64,
    total_orbit_calcs: u64,
...
struct TransmitResponse {
    ...
    model_diff_size_bytes: u64,
    compression_ratio: f64,
    estimated_transit_secs: f64,
    light_delay_secs: f64,
    status: String,
    elapsed_us: u128,
}

#[derive(Deserialize)]
struct ModelDiffRequest {
    base_model_id: String,
    current_state: serde_json::Value,
}
#[derive(Serialize)]
struct ModelDiffResponse {
    diff_id: String,
    base_model_id: String,
    diff_size_bytes: u64,
    full_state_size_bytes: u64,
    savings_pct: f64,
    elapsed_us: u128,
}
...
struct OrbitResponse {
    ...
    period_secs: f64,
    velocity_km_s: f64,
    communication_windows: Vec<CommWindow>,
}
#[derive(Serialize)]
struct CommWindow {
    start: String,
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "space_engine=info".into()),
        )
        .init();
...
async fn health(State(s): State<Arc<AppState>>) -> Json<Health> {
    ...
    Json(Health {
        ...
        total_ops: st.total_transmissions + st.total_model_diffs,
    })
}
...
async fn transmit(
    ...
    let transit = light_delay + (diff_size as f64 / 1000.0); // transmission time
    {
        ...
        st.total_transmissions += 1;
        st.bytes_transmitted += diff_size;
    }
    Json(TransmitResponse {
        transmission_id: uuid::Uuid::new_v4().to_string(),
        mission_id: req.mission_id,
        original_size_bytes: orig,
        model_diff_size_bytes: diff_size,
        compression_ratio: orig as f64 / diff_size as f64,
        estimated_transit_secs: transit,
...
async fn model_diff(
    State(s): State<Arc<AppState>>,
    Json(req): Json<ModelDiffRequest>,
) -> Json<ModelDiffResponse> {
    let t = Instant::now();
    let full_size = serde_json::to_string(&req.current_state)
        .map(|s| s.len() as u64)
        .unwrap_or(5000);
    ...
    Json(ModelDiffResponse {
        ...
        savings_pct: (1.0 - diff_size as f64 / full_size as f64) * 100.0,
        elapsed_us: t.elapsed().as_micros(),
    })
}
...
async fn orbit_calc(
    ...
    let windows = vec![
        CommWindow {
            start: "2026-02-23T06:00:00Z".into(),
            end: "2026-02-23T06:15:00Z".into(),
            duration_secs: 900,
            max_data_rate_kbps: 256.0,
            elevation_deg: 45.0,
        },
        CommWindow {
            start: "2026-02-23T18:30:00Z".into(),
            end: "2026-02-23T18:42:00Z".into(),
            duration_secs: 720,
            max_data_rate_kbps: 128.0,
            elevation_deg: 30.0,
        },
    ];
    s.stats.lock().unwrap().total_orbit_calcs += 1;
    Json(OrbitResponse {
        orbit_id: uuid::Uuid::new_v4().to_string(),
        body,
        altitude_km: alt,
        period_secs: period,
...
async fn create_mission(
    ...
    Json(MissionResponse {
        ...
        distance_au: dist,
        light_delay_secs: light_delay,
        optimal_protocol: "dtn-model-diff".into(),
...
async fn stats(State(s): State<Arc<AppState>>) -> Json<StatsResponse> {
    let st = s.stats.lock().unwrap();
    Json(StatsResponse {
        total_transmissions: st.total_transmissions,
        total_model_diffs: st.total_model_diffs,
        total_orbit_calcs: st.total_orbit_calcs,
        bytes_transmitted: st.bytes_transmitted,
        bandwidth_saved_pct: 95.0,
    })
}
